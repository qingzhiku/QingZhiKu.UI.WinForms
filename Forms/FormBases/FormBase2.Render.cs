    ...
    public partial class FormBase2
    {
        ...
        protected virtual CreateParams UpdateCreateParams(CreateParams param)
        {
            if (!DesignMode)
            {
                ...
                // 一个按钮也是窗口
                param.Style &= ~Win32.WS_CLIPCHILDREN;
        ...
        protected virtual void WM_NCCALCSIZE(Win32.RECT gap, ref Message m)
        {
            ...
            if (m.WParam.ToInt32() == 1)
            {
                ...
                var correntgap = (this.GetWindowState() == FormWindowState.Maximized && !OSFeature.Feature.IsWindows11OrGreater() ? 1 : 0);
        ...
        protected virtual void AdjustAeroPadding()
        {
            ...
            {
                // 排除win11以上
                if (!OSFeature.Feature.IsWindows11OrGreater())
                {
        ...
        protected virtual void WM_PAINT(ref Message m)
        {
            ...
            //    return;
            if (!IsHandleCreated || DesignMode || OSFeature.Feature.IsWindows11OrGreater() || this.WindowState != FormWindowState.Normal)
                return;
